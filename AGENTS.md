...
- **AI Takeoff Assistant** lives under Tenders → **AI Takeoff** tab (`TenderAiTakeoffPanel`). APIs: `GET/POST /api/tenders/[id]/ai-takeoff` and `POST .../ai-takeoff/apply`. AI proposes quantities via tools; NeXa calc in `ai-takeoff-calc.ts` owns money. Apply appends a BoQ sheet via `importBoqLinesIntoTender`.
...
- Hub schedule clashes are hard-blocked on `PUT /api/hub-state` (`SCHEDULE_CLASH` 409). What-if apply in the planner also preflights with `assertNoHubScheduleClashes` before writing live plans.
- PDF chrome omits placeholder bank details and blank/demo VAT·company lines (`isPlaceholderBankDetails` / `isPlaceholderCompanyRegistration` in `commercial-safeguards.ts`).
