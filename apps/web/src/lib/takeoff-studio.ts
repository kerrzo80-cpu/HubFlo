...
export function importPipeRunsIntoStudio(
  ...
  if (options?.replaceExistingAiPipes) {
    const removedPipeIds = new Set(
      next.geometries
        .filter((geo) => geo.kind === "linear" && isAiStudioGeometry(geo))
        .map((geo) => geo.id),
    );
    keep = next.geometries.filter((geo) => {
      if (geo.kind === "linear" && isAiStudioGeometry(geo)) return false;
      if (
        geo.kind === "count"
        && geo.autoGenerated
        && geo.linkedLinearId
...
export function importSkillCountsIntoStudio(
  ...
  // Only replace AI count pins — never wipe Blake/vision pipe runs (or their fittings).
  const keep = options?.replaceExistingAi
    ? studio.geometries.filter((geo) => !(geo.kind === "count" && isAiStudioGeometry(geo) && !geo.autoGenerated))
    : studio.geometries;
