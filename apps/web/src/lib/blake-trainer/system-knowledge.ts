...
export const nexaSystemKnowledge: SystemKnowledgeTopic[] = [
  ...
  },
  {
    ...
    content: [
      ...
      "Open the tender, import their BoQ on the Bill tab, then tick lines and run Ayla budget prices (rate library first, UK trade ballpark for gaps).",
      "Ask Ayla from the sidebar while the tender (or a job raised from it) is open — type back and forth about this BoQ and the files on the record. He can fill blank rates after you confirm. That is not a ChatGPT dump of six PDFs.",
    ...
    teachScript:
      "Tenders is the client bill. Import their BoQ, use Ayla budget prices as guides, then Ask Ayla on the open record to talk through blanks and FoT. Confirm before he writes rates. Won tenders become jobs.",
  ...
  },
  {
    ...
    tags: ["field", "tabs", "ui"],
    content: [
      "Approved Field bottom tabs: My Day, Ask Ayla, Hours, Connect.",
      "My Day = today’s jobs. Ask Ayla = on-site fault help. Hours = end-of-day time check. Connect = link/settings to Core.",
      "Do not invent extra Field tabs beyond this approved bar.",
    ].join(" "),
    mediaUrl: "/brand/nexa-command-mark.svg",
    teachScript:
      "Field has four tabs: My Day, Ask Ayla, Hours and Connect. Learn those four first — that’s the whole bottom bar.",
  ...
  },
  {
    ...
    content: [
      ...
      "Ask Ayla does not replace Gas Safe competence or company method statements.",
    ...
    teachScript:
      "Ask Ayla helps with faults — type or use Talk. Attach photos if you have them. It doesn’t replace your ticket or method statements.",
    checkPoints: ["ask blake", "photos", "talk", "fault", "method"],
    checkPrompt: "How do you use Ask Ayla on site, and what does it not replace?",
  },
  {
    id: "sys-field-hours",
    title: "Field — Hours",
...
export function generateBlakeTrainerCatalog(options?: {
  ...
  const modules: TrainerModule[] = [
    ...
    moduleFromTopics(
      "mod-field-complete",
      "Field complete",
      "My Day, Ask Ayla, Hours and the Field tab bar.",
