...
function logoBytes() {
  const uploaded = readBrandingAsset("logo-survey") || readBrandingAsset("logo");
  if (uploaded?.buffer) return uploaded;
  const brand = normalizeBusinessBranding(getHubDetailState().businessSettings);
  if (!/errol watson/i.test(`${brand.companyName} ${brand.tradingName}`)) return null;
  const candidates = [
    path.join(process.cwd(), "public", "ewg-logo.png"),
    path.join(process.cwd(), "apps", "web", "public", "ewg-logo.png"),
  ];
  const file = candidates.find(existsSync);
  return file ? { buffer: readFileSync(file), mimeType: "image/png" } : null;
}

async function embedLogo(pdf: PDFDocument) {
  const asset = logoBytes();
  if (!asset) return undefined;
  try {
    if (asset.mimeType.includes("png")) return pdf.embedPng(asset.buffer);
    return pdf.embedJpg(asset.buffer);
  } catch {
    return undefined;
  }
}
...
export async function createSurveyPdf(
  ...
  const company = displayCompanyName(getHubDetailState().businessSettings);
  ...
  function header(target: PDFPage) {
    ...
    if (logo) {
      ...
      target.drawText(company.toUpperCase() || "COMPANY", { x: margin, y: PageSizes.A4[1] - 45, font: bold, size: 14, color: darkBlue });
    }
  ...
  pages.forEach((pdfPage, index) => {
    ...
    pdfPage.drawText(`${company} | Generated by NeXa Surveyor`, { x: margin, y: 22, font: regular, size: 7, color: muted });
