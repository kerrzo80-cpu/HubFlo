...
export function buildAutoFittingsForLinear(
  ...
  const layerId = linear.layerId || resolveStudioDrawLayer(studio, linear.classificationId);
  if (autoElbows) {
...
export function appendLinearWithAutoFittings(
  ...
  const nextLinear: Extract<StudioGeometry, { kind: "linear" }> = {
    ...
    layerId: linear.layerId || resolveStudioDrawLayer(withClasses, linear.classificationId),
  };
...
export const UNSPECIFIED_FLOOR_LABEL = "Unspecified floor";

export function isUnspecifiedFloorLabel(label: string | undefined | null): boolean {
  const key = String(label || "").trim().toLowerCase();
  return key === "unspecified floor" || key === "unspecified";
}
...
export function inferFloorLabelFromDrawingName(fileName: string | undefined | null): string {
  const raw = String(fileName || "")
    ...
    .trim()
    .toLowerCase();
  if (!raw) return UNSPECIFIED_FLOOR_LABEL;
  ...
  if (/\bflat\s*([a-z0-9]+)\b/.test(raw)) {
    ...
    return match ? `Flat ${match[1]!.toUpperCase()}` : UNSPECIFIED_FLOOR_LABEL;
  }
  return UNSPECIFIED_FLOOR_LABEL;
}
...
export function layerForGeometry(studio: StudioState, geo: StudioGeometry): StudioServiceLayerId {
  const stamped = (geo.layerId || "").trim();
  if (stamped && stamped !== "all") return stamped;
  if (geo.kind === "count" && geo.autoGenerated && geo.linkedLinearId) {
    const parent = studio.geometries.find((row) => row.id === geo.linkedLinearId);
    if (parent) {
      const parentStamped = (parent.layerId || "").trim();
      if (parentStamped && parentStamped !== "all") return parentStamped;
...
export function summariseStudioBoq(
  ...
  for (const geo of studio.geometries) {
    ...
    const mpu = scale?.metresPerUnit || 0;
    const cls = studio.classifications.find((row) => row.id === geo.classificationId);
    const inferredFloor = splitByFloor
      ? inferFloorLabelFromDrawingName(documentNameLookup(options?.documentNames, geo.documentId))
      : "";
    const floorLabel = isUnspecifiedFloorLabel(inferredFloor) ? "" : inferredFloor;
