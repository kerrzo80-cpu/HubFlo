...
export type StudioBoqRow = {
  ...
  layerLabel: string;
  /** House type / drawing-set folder (Belerno, Bell, …). */
  setLabel?: string;
...
export type SummariseStudioBoqOptions = {
  ...
  /** Map of drawing documentId → house type (Belerno, Bell). Splits quantities per house. */
  documentSetLabels?: Record<string, string> | Map<string, string>;
  /** Show only this house type when set. */
  setFilter?: string;
};
...
function documentSetLookup(
  documentSetLabels: SummariseStudioBoqOptions["documentSetLabels"] | undefined,
  documentId: string,
): string {
  if (!documentSetLabels) return "";
  if (documentSetLabels instanceof Map) return documentSetLabels.get(documentId) || "";
  return documentSetLabels[documentId] || "";
}
 ...
 * When documentSetLabels are provided, quantities are split by house type (Belerno, Bell, …).
 */
export function summariseStudioBoq(
  ...
  const splitBySet = Boolean(options?.documentSetLabels);
  const setFilter = options?.setFilter?.trim() || "";
  ...
  const bump = (
    ...
    if (setFilter && setLabel !== setFilter) return;
  ...
  for (const geo of studio.geometries) {
    ...
    const floorLabel = splitByFloor
      ...
      : "";
    const setLabel = splitBySet
      ? documentSetLookup(options?.documentSetLabels, geo.documentId) || "Unassigned"
      : "";
    ...
    if (geo.kind === "linear") {
      ...
      const baseDescription = sized
        ...
        : (cls?.name || "Pipe run");
      if (metres > 0) {
        const key = `pipe|${setLabel}|${layerId}|${floorLabel}|${geo.material || ""}|${geo.diameter || ""}|${cls?.id || "run"}`;
        bump(key, layerId, setLabel, floorLabel, "Pipework", baseDescription, metres, "m", drop.verticalM, drop.noteLabel);
      }
    ...
    if (geo.kind === "count" && geo.autoGenerated && geo.fittingKind) {
      ...
      const key = `fit|${setLabel}|${layerId}|${floorLabel}|${geo.material || ""}|${geo.diameter || ""}|${geo.fittingKind}`;
      bump(key, layerId, setLabel, floorLabel, "Fittings", description, 1, "nr");
      continue;
    }

    if (geo.kind === "count") {
      const description = cls?.name || "Count";
      const key = `count|${setLabel}|${layerId}|${floorLabel}|${cls?.id || geo.classificationId}`;
      bump(key, layerId, setLabel, floorLabel, "Counts", description, 1, "nr");
      continue;
    }
    ...
    if (geo.kind === "area" && geo.closed) {
      ...
      const key = `area|${setLabel}|${layerId}|${floorLabel}|${cls?.id || geo.classificationId}`;
      bump(key, layerId, setLabel, floorLabel, "Areas", description, area, "m2");
    }
  }
  ...
  return [...groups.entries()]
    .map(([key, row]) => {
      ...
      return {
        ...
        setLabel: row.setLabel || undefined,
    ...
    .sort((a, b) => {
      const setDiff = (a.setLabel || "").localeCompare(b.setLabel || "");
