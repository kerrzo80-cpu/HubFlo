...
export function previewFittingsForDraft(
  ...
  const rise = clampRiseDropM(options.riseDropM);
  ...
  const riseElbow = options.autoElbows === false || !(rise > 0) ? 0 : 1;
  const elbows = planElbows + riseElbow;
  const couplings =
...
export function buildAutoFittingsForLinear(
  ...
  const riseDropM = clampRiseDropM(linear.riseDropM);
  ...
  if (autoElbows) {
    ...
    // Ceiling ↔ wall (or floor) transition needs an elbow when vertical metres are set.
    if (riseDropM > 0) {
      const end = linear.points[linear.points.length - 1];
      if (end) {
        fittings.push({
          id: `${linear.id}-elbow-rise`,
          classificationId: FITTING_ELBOW_CLASS_ID,
          kind: "count",
          documentId: linear.documentId,
          page: linear.page,
...
export function summariseStudioBoq(
  ...
  const bump = (
    ...
    const current = groups.get(key) || { description, quantity: 0, verticalM: 0, layerId, section, unit };
  ...
  for (const geo of studio.geometries) {
    ...
    if (geo.kind === "linear") {
      const rise = clampRiseDropM(geo.riseDropM);
      const metres = linearMeasuredMetres(geo.points, mpu, rise);
      ...
      if (metres > 0) {
        ...
        bump(key, layerId, "Pipework", baseDescription, metres, "m", rise);
      }
  ...
  return [...groups.entries()]
    .map(([key, row]) => {
      const dropNote = row.section === "Pipework" && row.verticalM > 0
