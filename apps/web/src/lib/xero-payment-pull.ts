...
export type XeroPullInvoiceInput = {
  id: string;
  ref: string;
  chargeTotal: number;
  vatRate: number;
  status?: string;
  claimType?: string;
  payments?: XeroLedgerPayment[];
  paidAmount?: number;
  paymentStatus?: string;
  xeroInvoiceId?: string;
};

export type XeroPaymentPullResult = {
  ok: true;
  invoiceId: string;
  invoiceRef: string;
  match: {
  ...
  addedPayments: XeroLedgerPayment[];
  payments: XeroLedgerPayment[];
  paidAmount: number;
...
export async function resolveXeroInvoiceForPull(invoiceNumber: string, preferredId?: string) {
  const accessToken = await resolveXeroAccessToken();
  const tenantId = getStoredXeroTenantId();
  if (!accessToken || !tenantId) {
    throw new Error(
      "Xero live token and tenant are required to pull payments. Connect Xero in Setup or set XERO_ACCESS_TOKEN + XERO_TENANT_ID.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(body.Detail || body.Message || `Xero invoice lookup failed (${response.status}).`);
  }
...
export function validateInvoiceForXeroPaymentPull(invoice: XeroPullInvoiceInput) {
  if (!invoice.id || !invoice.ref) {
    return "Invoice id and ref are required.";
  }
  if (invoice.claimType === "valuation") {
    return "Convert the valuation to a progress claim before pulling Xero payments.";
  }
  if (invoice.status === "Draft" || invoice.status === "Cancelled") {
    return "Draft or cancelled invoices cannot pull Xero payments.";
  }
  return null;
}

export async function pullXeroPaymentsForInvoice(invoice: XeroPullInvoiceInput): Promise<XeroPaymentPullResult> {
  const validationError = validateInvoiceForXeroPaymentPull(invoice);
  if (validationError) {
  ...
  if (!xeroInvoiceId) {
    throw new Error("Xero invoice is missing InvoiceID.");
  }
  ...
  for (const row of xeroPayments) {
    ...
    const alreadyLinkedSumUp = existing.some(
      (payment) =>
        payment.source === "sumup" && (payment as { xeroPaymentId?: string }).xeroPaymentId === paymentId,
    );
  ...
  const paymentStatus: XeroPaymentPullResult["paymentStatus"] =
