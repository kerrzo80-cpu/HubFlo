...
export default function GuidedSurveyPage() {
  ...
  async function completeCurrentSurvey() {
    ...
    setReview(body.review || null);
    setNotice("Survey completed and ready for Estimator.");
  }
    ...
    <main className="guided-survey-app">
      ...
      <div className="guided-survey-shell">
        ...
        <section className="guided-workspace">
          ...
          {activeStep === "details" ? (
            <section className="guided-form-section">
              ...
              <div className="guided-form-grid">
                ...
                <label className="wide">Customer requirements<textarea value={survey.customerRequirements} onChange={(event) => queuePatch({ customerRequirements: event.target.value })} placeholder="What outcome has the customer requested?" /></label>
              </div>
            </section>
          ) : null}
          ...
          {activeStep === "scope" ? (
            <section className="guided-form-section">
              <div className="guided-section-title"><Wrench size={18} /><span><h2>Proposed scope</h2><p>Add each task as a separate, traceable item.</p></span></div>
          ...
          {activeStep === "review" ? (
            <section className="guided-review">
              <div className="guided-review-banner" data-ready={review?.canComplete ? "true" : "false"}>{review?.canComplete ? <CheckCircle2 size={24} /> : <AlertTriangle size={24} />}<span><h2>{review?.canComplete ? "Essential checks passed" : "Completion checks need attention"}</h2><p>Unknowns can remain TBC when their reason is recorded. Safety, identity and job-link items must be resolved.</p></span></div>
              ...
              <div className="guided-review-actions"><button type="button" onClick={() => void loadReview()}><ClipboardCheck size={16} /> Refresh review</button><a href={`/api/surveys/${encodeURIComponent(survey.id)}/pdf`} target="_blank" rel="noreferrer"><Download size={16} /> Survey PDF</a>{survey.status === "Complete" || survey.status === "Sent to estimator" ? <button className="guided-primary-action" type="button" onClick={() => void sendToEstimator()}><Send size={16} /> Open in Estimator</button> : <button className="guided-primary-action" type="button" disabled={!review?.canComplete} onClick={() => void completeCurrentSurvey()}><CheckCircle2 size={16} /> Complete survey</button>}</div>
            </section>
          ) : null}
