...
export default function StudioCanvas({
  ...
  useEffect(() => {
    ...
    for (const geo of geos) {
      ...
      if (geo.kind === "count") {
        ...
        if (geo.autoGenerated && geo.fittingKind === "90-elbow") {
          const isDrop = /rise\/drop/i.test(geo.sourceText || "");
          if (isDrop) {
            // Drop markers: inverted tee / stub down the page (distinct from plan bend diamonds).
            const s = selected ? 8 : 6;
            ctx.beginPath();
            ctx.moveTo(point.x - s, point.y - s * 0.35);
            ctx.lineTo(point.x + s, point.y - s * 0.35);
            ctx.lineTo(point.x + s, point.y + s * 0.15);
            ctx.lineTo(point.x + s * 0.35, point.y + s * 0.15);
            ctx.lineTo(point.x + s * 0.35, point.y + s);
            ctx.lineTo(point.x - s * 0.35, point.y + s);
            ctx.lineTo(point.x - s * 0.35, point.y + s * 0.15);
            ctx.lineTo(point.x - s, point.y + s * 0.15);
            ctx.closePath();
            ctx.fillStyle = selected ? colour : `${colour}ee`;
            ctx.fill();
            ctx.strokeStyle = "#fff";
            ctx.lineWidth = 1.4;
            ctx.stroke();
          } else {
            const s = selected ? 9 : 7;
            ctx.beginPath();
            ...
            ctx.lineTo(point.x - s, point.y);
            ctx.closePath();
            ctx.fillStyle = selected ? colour : `${colour}dd`;
            ctx.fill();
            ctx.strokeStyle = "#fff";
            ctx.lineWidth = 1.5;
            ctx.stroke();
          }
        } else if (geo.autoGenerated && geo.fittingKind === "coupling") {
          const r = selected ? 6 : 5;
          ctx.beginPath();
          ctx.rect(point.x - r, point.y - r * 0.55, r * 2, r * 1.1);
          ctx.fillStyle = selected ? colour : `${colour}dd`;
          ctx.fill();
          ctx.strokeStyle = "#fff";
          ctx.lineWidth = 1.4;
  ...
  function finishLinear(points = draftPoints) {
    ...
    const geo: Extract<StudioGeometry, { kind: "linear" }> = {
      id: studioId("geo"),
      classificationId: activeClass.id,
      kind: "linear",
      documentId: document.id,
  ...
  function deleteSelected() {
    ...
    if (selected?.kind === "linear") {
      onChange(removeLinearAndFittings(studio, selectedId));
    } else if (selected?.kind === "count" && selected.autoGenerated && selected.linkedLinearId) {
      // Don't orphan-delete one auto fitting — regenerate from parent run instead.
      const linear = studio.geometries.find(
        (geo) => geo.kind === "linear" && geo.id === selected.linkedLinearId,
      );
      if (linear && linear.kind === "linear") {
        onChange(appendLinearWithAutoFittings(
          { ...studio, geometries: studio.geometries.filter((geo) => geo.id !== selectedId) },
          linear,
        ));
      } else {
  ...
  const toolHelp: Record<StudioTool, { label: string; title: string; hint: string }> = {
    ...
    linear: {
      ...
      hint: (() => {
        const spec = pipeSpecById(studio.activePipeSpecId);
        if (!activeClass) return "Pick a service under Draw as, choose Size, then tap along the run.";
        const bits = [
          activeClass.name,
          `${spec.diameter} ${spec.material}`,
        ];
  ...
  return (
    <div className="nexa-studio-canvas-wrap">
      ...
      <div className="nexa-studio-statusbar">
        <span className="nexa-studio-statusbar-hint" title={draftHint}>
          {draftHint}
        </span>
        <span>
          Scale:{" "}
        ...
        {selectedLinear && selectedDrop ? (
          ...
          >
            <span className="nexa-studio-rise-form-title">Drops</span>
          ...
          <p className="nexa-studio-drop-prompt" title="Select a Length run in Edit to set Drops × Height. Use 3D / Iso in the toolbar for isometric preview.">
            {pageHasDrops
              ? "Edit a Length run to change Drops × Height · 3D / Iso in toolbar"
              : "Edit a Length run → Drops × Height · 3D / Iso in toolbar"}
          </p>
        ) : null}
