...
function releaseMarkupPointer(target: SVGSVGElement, pointerId: number) {
  ...
  function linkedMarkupSymbolsForMovedPipe(pipeId: string, dx: number, dy: number) {
  ...
  function hasMatchingAutoFittingAt(
    ...
    return [...existingSymbols, ...pendingSymbols].some((symbol) => {
      if (symbol.autoGenerated && symbol.linkedPipeId && symbol.linkedPipeId !== pipe.id) {
        return false;
      }
      return (
        symbol.kind === kind
    ...
    junctions.forEach(({ junction, adjacent }) => {
      ...
      const matchesRightAngle = contextPipes.some((existingPipe) => {
        ...
        for (let index = 1; index < existingPipe.points.length; index += 1) {
          ...
          if (!previous || !current) continue;
          const closest = closestMarkupPointOnSegment(junction, previous, current);
          if (markupPointDistance(junction, closest) > tolerance) continue;
          const previousAngle = markupPointDistance(previous, junction) > 8 && isMarkupRightAngle(previous, junction, adjacent);
          const currentAngle = markupPointDistance(current, junction) > 8 && isMarkupRightAngle(current, junction, adjacent);
  ...
  async function saveMarkedDrawingForEngineers() {
    ...
    const exportLayerLabel = activeMarkupLayerId === "all" ? "Master all layers" : activeMarkupLayerLabel;
    ...
    const symbolSvg = snapshotSymbols.map((symbol) => {
      ...
      const labelSvg = symbol.autoGenerated
        ? ""
        : `<text x="10" y="4" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${colour}" paint-order="stroke" stroke="#fff" stroke-width="3">${label}</text>`;
      if (symbol.category === "Valve") {
        return `<g transform="translate(${symbol.x} ${symbol.y}) rotate(${symbol.rotation})"><path d="M-6 0 L0 -6 L6 0 L0 6 Z" fill="#fff" stroke="${colour}" stroke-width="2" />${labelSvg}</g>`;
      }
      if (symbol.category === "Plant") {
        return `<g transform="translate(${symbol.x} ${symbol.y}) rotate(${symbol.rotation})"><rect x="-8" y="-6" width="16" height="12" rx="2" fill="#fff" stroke="${colour}" stroke-width="2" />${labelSvg}</g>`;
      }
      return `<g transform="translate(${symbol.x} ${symbol.y}) rotate(${symbol.rotation})"><circle cx="0" cy="0" r="${symbol.autoGenerated ? 4 : 6}" fill="#fff" stroke="${colour}" stroke-width="2" />${labelSvg}</g>`;
    }).join("");
