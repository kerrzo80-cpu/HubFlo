...
export function JobCard({ job }: { job: FieldScheduleItem }) {
  ...
  return (
    <article className="job-card">
      <Link href={`/jobs/${job.scheduleId}`} className="job-card-main">
      ...
      <div className="job-actions">
        <Link href={`/jobs/${job.scheduleId}?tab=pack`}>
          <Layers size={16} /> Pack
        </Link>
        <Link href={`/jobs/${job.scheduleId}?tab=checklist`}>
          <ClipboardCheck size={16} /> Checklist
        </Link>
        <Link href={`/jobs/${job.scheduleId}?tab=photos`}>
